...
function meuCodigo() {
    ...
    // O teorema de Pitágoras se verifica?
    const TOLERANCIA = 0.0001;  // para compensar arredondamentos

    const maiorAoQuadrado = Math.pow(lados[0], 2);
    const demaisAoQuadrado = Math.pow(lados[1], 2) + Math.pow(lados[2], 2);
    let resposta = '';

    if ((maiorAoQuadrado - demaisAoQuadrado) < TOLERANCIA) {
        resposta = 'Sim, é um triângulo retângulo';
    } else {
        resposta = 'Não é um triângulo retângulo';
    }

    tela.escreve(new Coords(50, 50), `Lados: ${lados[0]}, ${lados[1]} e ${lados[2]}`);
