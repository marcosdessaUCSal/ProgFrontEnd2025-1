...
function meuCodigo() {
    ...
    // UTILIZANDO AS REGRAS DESTE SEMESTRE PARA A NOSSA DISCIPLINA

    const MED_MIN = 4;
    const MED_APROV = 6;
    const TOLERANCIA_MEDIA = 5.8;
    ...
    media = (n1 + n2) / 2;
    // arredondamento?
    if (media >= TOLERANCIA_MEDIA && media < 6) {
        media = MED_APROV;
    }
    ...
    switch(true) {
        case (media < MED_MIN):
            resposta = 'reprovado';
            break;
        case (media < MED_APROV):
            resposta = 'apto à Prova Final';
            break;
        default:
            resposta = 'aprovado';
    }
